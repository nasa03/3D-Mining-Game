...
public class ShopItemScript : MonoBehaviour {
    ...
    void Start () {
        shopManager = GameObject.Find("GameManager").GetComponent<ShopSystem>();
            ...
            if (hoverPanel == null)
            {
                ...
                for (int i = -1; i < oreName.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (GameObject.Find("GameManager").GetComponent<InventorySystem>().isItemAvailable(oreName[i], oreAmount[i]))
                        {
                            newText.GetComponent<Text>().color = Color.green;
                        }
                        else
    ...
    public void onClick()
    {
        ...
        for (int i = -1; i < oreName.Count; i++)
        {
            ...
            else
            {
                ...
                if (GameObject.Find("GameManager").GetComponent<InventorySystem>().isItemAvailable(oreName[i], tempStoreAmount[i]))
                {
                    newText.GetComponent<Text>().color = Color.green;
                    a++;
                }
